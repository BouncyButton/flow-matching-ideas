...
def main():
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch in tqdm(train_loader, desc=f"Epoch {epoch + 1}/{num_epochs}", leave=False):
            src_batch = batch["images"].to(device)
            tgt_batch = batch["masks"].to(device)
            ...
            x_0 = src_batch  # torch.randn_like(im_batch)
            t = torch.rand(src_batch.shape[0], device=device)

            # Sample the path from x_0 to x_batch
            sample_info = path.sample(t=t, x_0=x_0, x_1=tgt_batch)

            # Predict velocity and compute loss
            v_pred = model(
                x=sample_info.x_t,
                t=sample_info.t,
                masks=None,  # tgt_batch
                cond=classes_batch
            )
